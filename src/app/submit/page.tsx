...
export default function SubmissionPage() {
  ...
  const handleAnswerChange = (questionId: string, value: string | number | string[] | boolean) => {
    setAnswers(prev => ({
      ...prev,
      [questionId]: value
    }))
  ...
  const validateField = (fieldId: string, value: unknown, validation: { type: string; pattern?: string; message?: string }) => {
    let isValid = true
    let errorMessage = ""
    ...
    switch (validation.type) {
      ...
      case "range":
        ...
        break
      case "currency":
        isValid = /^\d+(\.\d{2})?$/.test(strValue) && parseFloat(strValue) > 0
        errorMessage = validation.message || "Invalid currency amount"
        break
      case "pattern":
        if (validation.pattern) {
          isValid = new RegExp(validation.pattern).test(strValue)
  ...
  const renderQuestion = (question: Question) => {
    ...
    switch (question.type) {
        ...
        return (
          <div className="space-y-2">
            <Textarea
              ...
              rows={3}
              maxLength={question.constraints?.maxLength as number | undefined}
            />
            {validationError && (
              <p className="text-sm text-red-600">{validationError}</p>
            )}
            {typeof question.constraints?.maxLength === 'number' && (
              <p className="text-xs text-muted-foreground">
                {answerStr.length || 0}/{String(question.constraints.maxLength)} characters
              </p>
            )}
          </div>
        )
      ...
      case "multiple_choice":
        return (
          <Select value={answerStr} onValueChange={(value) => handleAnswerChange(question.id, value)}>
            <SelectTrigger>
            ...
            {question.options?.map((option, index) => (
              <div key={index} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  checked={answerArr.includes(option) || false}
                  onCheckedChange={(checked) => {
                    const newValues = checked
                      ? [...answerArr, option]
                      : answerArr.filter((v: string) => v !== option)
            ...
            {answerStr && (
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm">{String(answer)}</span>
                </div>
      ...
      case "signature":
        const signature = answerObj ? signatures.find(s => s.id === answerObj.id) : undefined
        return (
      ...
      case "data_integration":
        const integration = dataIntegrations.find(di => di.source === question.dataSource)
        return (
          <div className="space-y-3">
