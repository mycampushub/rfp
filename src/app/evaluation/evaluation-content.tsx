...
export function EvaluationContent({ evaluations }: { evaluations: EvaluationData[] }) {
  ...
  const getScoreStars = (score: number, maxScore: number) => {
    ...
    const filledStars = maxScore > 0 ? Math.round((score / maxScore) * 5) : 0
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {evaluations.filter(e => e.status === "completed" || e.status === "finalized").length}
            </div>
          </CardContent>
        </Card>
        <Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {evaluation.deadline ? new Date(evaluation.deadline).toLocaleDateString() : "Not set"}
                    </div>
                  </div>
                </div>
            ...
            <TableBody>
              {evaluations.map((evaluation) => (
                <TableRow key={evaluation.id}>
                  ...
                  <TableCell>
                    <div className="text-sm">
                      {evaluation.deadline ? new Date(evaluation.deadline).toLocaleDateString() : "-"}
                    </div>
                  </TableCell>
                  <TableCell>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {evaluations
              .filter(e => e.status === "completed" || e.status === "finalized")
            ...
            {evaluations.filter(e => e.status === "completed" || e.status === "finalized").length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-4">No completed evaluations to rank.</p>
            )}
          </div>
        </CardContent>
      </Card>
